...
#pragma clang diagnostic pop
        ...
        if (numberOfGaps >= (numberOfPoints - 1)) {
            ...
            if (self.positionYAxisRight) {
                xAxisXPositionFirstOffset = 15;
                xAxisXPositionLastOffset = xAxisXPositionFirstOffset + 1 + viewWidth/2;
            } else {
                xAxisXPositionFirstOffset = 15+self.YAxisLabelXOffset;
                xAxisXPositionLastOffset = viewWidth/2 + xAxisXPositionFirstOffset + 1 - 12;
            }
            UILabel *firstLabel = [self xAxisLabelWithText:firstXLabel atIndex:0];
            firstLabel.frame = CGRectMake(xAxisXPositionFirstOffset, self.frame.size.height-20, viewWidth/2 - 15, 20);
            ...
            lastLabel.frame = CGRectMake(xAxisXPositionLastOffset, self.frame.size.height-20, viewWidth/2 - 19, 20);
            ...
            UIView* line = [[UIView alloc] initWithFrame:CGRectMake(0, firstLabel.frame.origin.y, self.frame.size.width, .5)];
...
- (void)drawYAxis {
    ...
    if (self.autoScaleYAxis) {
        ...
        for (NSNumber *dotValue in dotValues) {
            ...
            float xCoordinate = 15;
//            labelYAxis.center = CGPointMake(xCoordinate, yAxisPosition);
            labelYAxis.center = CGPointMake(xCoordinate, yAxisPosition + (dotValue.floatValue == self.maxValue ? -15 : 15));
//            labelYAxis.center = CGPointMake(5, yAxisPosition);
//            float x = 5;
//            labelYAxis.frame.origin.
            ...
            UIView* line = [[UIView alloc] initWithFrame:CGRectMake(0, yAxisPosition, self.frame.size.width, .5)];
            line.backgroundColor = UIColor.darkGrayColor;
            line.alpha = .3;
            [self addSubview:line];
        }
...
//  CGRectMake(xAxisXPositionFirstOffset, self.frame.size.height-20, viewWidth/2, 20);
    ...
    CGFloat xOrigin = 15;
//    self.positionYAxisRight ? 0 : self.YAxisLabelXOffset;
    CGFloat viewWidth = self.frame.size.width - self.YAxisLabelXOffset - 30;
