//
//  ReportComplexListController.swift
//  ККВФ
//
...
class ReportComplexListController: UIViewController, UISearchBarDelegate, TableViewDelegate
{
    var mComplexList: NSArray!
    var mTitle: String = ""
    ...
    init(withComplexList:AnyObject, title: String)
    {
        super.init(nibName: nil, bundle: nil)
        if withComplexList is NSDictionary && (withComplexList as! NSDictionary).allKeys.count != 0
        {
            mComplexList = (withComplexList as! NSDictionary).allKeys as NSArray
        }
        else if withComplexList is NSArray && (withComplexList as! NSArray).count != 0
        {
            mComplexList = withComplexList as? NSArray
        }
        else
        {
            mNoData = true
    ...
    func addSearchBar()
    {
        ...
        mSearchBar = mSearchBar.setSearchBarWithFrame(searchBar: mSearchBar,
                                                      frame: CGRect(x: 10, y: 5, width: panel.frame.width - 20, height: panel.frame.height - 10),
    ...
    func addTable()
    {
        if !mNoData
        {
            mController = CoreDataStack.sharedInstance.getComplexesByNameOrAddress(nameOrAddress: "", ids: mComplexList)
    ...
    func searchBar(_ searchBar: UISearchBar, textDidChange searchText: String) {
        ...
        mController = CoreDataStack.sharedInstance.getComplexesByNameOrAddress(nameOrAddress: subString, ids: mComplexList)
        mTable.setFetchController(controller: mController, withDelegate: false)
        mTable.update()
    }
